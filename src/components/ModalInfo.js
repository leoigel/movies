...
const ModalInfo = ({ info }) => {
    ...
    return (
        <MainContent>
            <DivModal >
                <img src={`https://image.tmdb.org/t/p/original${info.backdrop_path}`} style={{width:'100%', height:'100%'}}/>
                <Modal width={width}>
                    <Details width={width}>

                        <ImgDiv width={width}>
                        
                            {info.poster_path?<img src={`https://image.tmdb.org/t/p/w92${info.poster_path}`} style={{ borderRadius: '5px', border: '1px solid #ffffff' }} />:<ImgCastNotFound />}
                        </ImgDiv>
                        <div>
...
left: 50%;
transform: ${props => {
        return props.width > 600 ? 'translate(-50%, -50%);' : 'translate(-50%, -40%);'
    }};
