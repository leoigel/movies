...
const Search = ({ location, match }) => {
      ...
      Promise.all(
        urls.map(async (url) => {
          const query = await fetch(
            `https://api.themoviedb.org/3/search/${url}?api_key=d98ee9811e179187b61f0f6b83bb3918&query=${category}`
          );
  ...
  useEffect(() => {
    async function generateCategory() {
      const query = await fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=d98ee9811e179187b61f0f6b83bb3918&query=${category}`
      );
      const responseQuery = await query.json();
