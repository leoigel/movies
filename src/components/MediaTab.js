...
const MediaTab = ({ params }) => {
    ...
    useEffect(() => {
        async function getmedia() {
            const media = await fetch(`https://api.themoviedb.org/3/movie/${params}/images?api_key=d98ee9811e179187b61f0f6b83bb3918&language=en-US&include_image_language=null`);
                ...
                <ConteinerImg>
                    {backgrounds.length !== 0 ? (
                        backgrounds.map(background => {
                            return (
                                <img src={`https://image.tmdb.org/t/p/w300${background.file_path}`} />
                            )
                        })
