...
const ChartPop = () => {
  ...
  useEffect(() => {
    async function mostPopularyMovies() {
      const popularyMovies = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=d98ee9811e179187b61f0f6b83bb3918&language=en-US&sort_by=${sortInput}&year=${year}&with_genres=${genresData}`
      );
