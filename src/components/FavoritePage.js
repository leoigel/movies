...
const FavoritePage = () => {
  const { windowDimensions, setFavoritesCard } = useMovies();
  const { width } = windowDimensions;

  const deleteCard = (id) => {
    const newFavoritesCards = JSON.parse(localStorage.getItem('cards')).filter(
      (favorites) => {
    ...
    localStorage.setItem(
      'cards',
      JSON.stringify([...newFavoritesCards] || '[]')
    );
                ...
                <CardsContainer key={card.id} width={width}>
                  ...
                  <ContainerInformation>
                    <ContainerDetails>
                      ...
                      <Div>
                        <Date>{`${moment(card.release_date).format(
                          'LL'
                        )}`}</Date>
