...
const CommentReview = ({ newContent, setNewCommnets, info }) => {
  ...
  const editAndSaveComment = (id, trueOrFalse) => {
    const openClose = newContent.map((item, index) => {
      {
        if (item.id === id) {
          return {
            id: item.id,
              ...
              {!answer.editing ? (
                <Paper elevation={3}>
                  <>
                    <div>
                      <Title>
                        <H3>{` A review by ${localStorage.getItem(
                          'userName'
                        )}`}</H3>
                        <H5>{`Written by ${localStorage.getItem(
                          'userName'
                        )}`}</H5>
                      </Title>
                      <IconPencil
                        className="fas fa-pencil-alt"
                        onClick={() => editAndSaveComment(answer.id, true)}
                      />
                      <IconTrash
                ...
                <Paper>
                  <Textarea
                    onChange={(e) => writeNewcontent(e, answer)}
                    style={{ rows: '4', cols: '50', maxlength: '50' }}
                  >
                    {answer.content}
                  </Textarea>
                  <IconSave
                    className="far fa-save"
                    onClick={() => editAndSaveComment(answer.id, false)}
                  />
                </Paper>
              )}
            </div>
